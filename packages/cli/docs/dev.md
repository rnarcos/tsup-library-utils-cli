...
Development workflow with automatic package.json management for seamless library development.
...
- `--workspaces <workspaces...>` - Workspace directories (default: `["packages"]`)
- `--single-repo` - Treat as single repository instead of monorepo
...
```bash
# ✅ Correct workflow
tsup-lib dev              # Switch to dev mode
# ... make changes ...
git add .
...
# ✅ For publishing
tsup-lib build            # Builds and switches to prod mode
npm publish               # Publish with prod package.json
tsup-lib dev              # Switch back to dev mode immediately
...
For standalone libraries (not in a monorepo):

- Processes only the current package
- Updates package.json for direct source imports
- Enables TypeScript development workflow

### Monorepo Mode (Default)

```bash
tsup-lib dev --workspaces packages actions configs
```

For monorepos with multiple packages:

- Processes all packages in specified workspaces
- Links packages to each other's source files
- Enables cross-package development
- Maintains workspace dependencies
...
- **Workspace changes** - Detects new packages
...
# Single repository
tsup-lib dev --single-repo --watch
```

### Monorepo Development

```bash
# Process default workspaces (packages/)
tsup-lib dev --watch

# Custom workspaces
tsup-lib dev --workspaces packages apps tools --watch

# From monorepo root
pnpm run dev  # Usually configured to run tsup-lib dev --watch
```
...
tsup-lib dev --watch --single-repo
...
# 7. Publish workflow
tsup-lib build    # Switch to prod mode & build
npm publish       # Publish
tsup-lib dev      # Switch back to dev mode
git add package.json
git commit -m "chore: restore dev mode"
```
...
tsup-lib dev --workspaces packages --verbose

# Check pnpm-workspace.yaml or package.json workspaces
```
...
- Use specific workspace directories instead of wildcards
- Exclude test directories from processing when possible
- Use `--single-repo` for standalone packages
- Consider using turbo for large monorepos
