...
/**
 * Check if a file should be included in public exports
...
/**
 * Get public files for exports with comprehensive error handling
 ...
 */
function getBinFilePaths(pkg, sourcePath) {
...
function filterBinFromPublicFiles(publicFiles, binPaths, sourcePath) {
  ...
  return Object.fromEntries(
    Object.entries(publicFiles).filter(([name, path]) => {
...
export function writePackageJson(rootPath, prod = false) {
  try {
    ...
    const getExports = (path) => {
      ...
      if (!prod) {
        // Development mode: formats point to source file based on what will be in package.json
        const sourceExport = `./${join(sourceDir, relativePath)}.js`;
...
export function makeGitignore(rootPath) {
  ...
  try {
    ...
    const buildArtifactsSection = [
      '# Build artifacts (auto-generated by tsup-lib)',
 ...
 */
function updateGitignoreWithBuildArtifacts(existingContent, buildArtifactsSection) {
  const startMarker = '# Build artifacts (auto-generated by tsup-lib)';
